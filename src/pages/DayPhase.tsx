...
export default function DayPhase() {
  ...
  const voters = alivePlayers.filter(p => p.hasVotingRights !== false);
              ...
              <div className="grid grid-cols-3 gap-2">
                {state.players.map(p => (
                  ...
                  >
                    ...
                    {p.name}
                    {p.alive && p.hasVotingRights === false && <div className="text-[10px] mt-0.5">(無投票權)</div>}
                  </div>
                ))}
              </div>
